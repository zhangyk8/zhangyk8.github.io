...
* `param1`, `param2` are function parameters/arguments for the user to input. We can assign default values by setting them equal to something in the function definition.
...
- **Abstract**: Identify the variables and decide if they are required or have defaults.
  ...
  - The function should have one input argument/parameter and one output value (with a possible message that is printed to the console).
...
Note: Our function `FahToCel` cannot handle the non-numeric input!
...
- The inputs for a function call are not evaluated until they are needed, i.e., only the needed objects in a function call will be loaded in memory and/or looked for.

- This lazy evaluation mechanism appears in other functional programming languages, not just in R.
...
Debugging is a the process of locating, understanding, and removing bugs from our code.
...
- Example: if our function breaks with a large data matrix of real-world data, consider creating a toy matrix, says, a $3\times 3$ matrix of 1's? 
...
- The main downside of using `print()` or `cat()` is that the output messages can get messy quickly if we use multiple print statements, and we cannot further investigate the object. However, many bugs come down to typos or other trivial errors, and printing statements are often all we need!
...
- Even for our own functions, it may be more convenient to debug the entire function rather than edit the source code. 

- When a function is in debug mode, it will remain in the debug mode until we call `undebug()`.
...
- We can use `test_that()` function in the `testthat` package to do unit tests. The code structure for the `test_that()` function goes as:
