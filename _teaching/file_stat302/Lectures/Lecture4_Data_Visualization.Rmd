...
- In `.Rmd` files, we can load all the required packages in the opening chunk and set the parameter `include = FALSE` to hide the messages and code.
...
We can load all the `tidyverse` packages into our current R session using
...
Recall that packages encapsulated functions written by different R developers.
...
- The main structure used by these packages is the data frame (or tibble, but we won't go there).
...
Example: We can write `exp(1)` with pipes as `1 %>% exp`, and `log(exp(1))` as `1 %>% exp %>% log`.
...
Example: We can subset top 1 row of the `mcars` data frame using
...
An alternative approach using `subset()` function: 
...
An alternative approaches using a basic R syntax:
...
To estimate a density from a numeric vector, we use the `density()` function.
