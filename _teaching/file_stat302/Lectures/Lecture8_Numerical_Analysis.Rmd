...
A function $f$ defined on a set $\mathcal{X}$ of real numbers has the **limit** $L$ at $x_0\in \mathcal{X}$, written $\lim_{x\to x_0} f(x) = L$, if, given any $\epsilon >0$, there exists a real number $\delta>0$ such that 
...
We denote it by $\lim_{n\to\infty} x_n =x$.
...
b. If $\{x_n\}_{n=1}^{\infty}$ is any sequence in $\mathcal{X}$ converging to $x_0$, then $\lim_{n\to\infty} f(x_n) = f(x_0)$.
...
- As $n\to \infty$, $\lim\limits_{n\to\infty} P_n(x)$ becomes an infinite Taylor series of $f$ at around point $x_0$.
...
Using the `eval()` function requires us to assign correct values to the R variables as in our expression.
...
- What is the base-10 decimal value of the 8-digit binary number **00110011**?
...
- General scientific notation in base-10: 
$$a\times 10^{b}.$$
...
Note: The computer cannot store 1/7 and $\pi$ because the total number of digits is limited.
...
- The algorithm or sequence that polynomially/sublinearly converges to $x^*$ is relatively slow, in the sense that
...
- The "Q" here stands for "quotient", while the "R" stands for "root".
...
Suppose that $\lim\limits_{h\to 0} g(h)=0$ with $g(h)>0$ for all $h\in \mathbb{R}$ and $\lim\limits_{h\to 0} f(h)=L$. If there exists a constant $C>0$ such that
...
# Caveats When Using the Bisection Method
...
- The choice of the initial interval affects not only the convergence speed but also the output approximated solution as well.
...
We will write a function `findRc()` to search for $R_c$, which is the smallest possible radius such that a given ad hoc network is completely connected.
