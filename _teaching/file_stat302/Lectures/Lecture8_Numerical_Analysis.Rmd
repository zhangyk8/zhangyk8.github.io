...
powerIter = function(mat, b_0 = NULL, tol = 1e-7) {
  ...
  curr_eig = (t(b_0) %*% mat %*% b_0)/(t(b_0) %*% b_0)
  eig_seq = c(curr_eig)
...
- We want to determine a polynomial of degree 1 that passes through (or _interpolate_) two distinct points $(x_0,f(x_0)), (x_1, f(x_1))$.
...
- By Weighted Mean Value Theorem for Integrals, there exists $\xi\in (x_0,x_1)$ such that when $x_0=a$, $x_1=b$, and $h=b-a$, 
\begin{align*}
...
\begin{align*}
\int_a^b f(x) dx &= \int_{x_0}^{x_2} \Big[ \frac{(x-x_1)(x-x_2)}{(x_0-x_1)(x_0-x_2)} f(x_0) + \frac{(x-x_0)(x-x_2)}{(x_1-x_0)(x_1-x_2)} f(x_1) \\
&\quad \quad + \frac{(x-x_0)(x-x_2)}{(x_2-x_0)(x_2-x_1)} f(x_2) \Big] dx \\
...
- High-degree formulas would be required, and the values of the coefﬁcients in these formulas are difficult to obtain.
...
&\approx e^{-\lambda g(x^*)} h(x^*) \int_{-\infty}^{\infty} \underbrace{e^{-\frac{\lambda g''(x^*)}{2}(x-x^*)^2}}_{\text{Density of } N\left(x^*, \frac{1}{\lambda g''(x^*)}\right)} dx \\
...
Submit Lab 7 on Canvas by the end of Thursday (December 7) and the final project by the end of December 13!!
